...
export default function Page() {
  ...
  const handleExportJson = () => {
    ...
    const dataStr = JSON.stringify(editGround, null, 2);
    const blob = new Blob([dataStr], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${editGround.name || 'ground'}_${new Date().toISOString().split('T')[0]}.json`;
    a.click();
  ...
  const handleImportJson = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        const importedGround = JSON.parse(event.target?.result as string);
        processImportedGround(importedGround);
      } catch {
        alert('无法解析 JSON 文件');
      }
    };
  ...
  const handleImportFromText = () => {
    ...
    try {
      const importedGround = JSON.parse(importJsonText);
      processImportedGround(importedGround);
  ...
  const processImportedGround = (importedGround: unknown) => {
    if (importedGround && typeof importedGround === 'object' && !Array.isArray(importedGround)) {
      const newGround = importedGround as GroundData;
      const newId = `ground-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`;
      newGround.id = newId;
      newGround.createdAt = new Date().toISOString().split('T')[0];
      newGround.updatedAt = new Date().toISOString().split('T')[0];
